...
"""
================================================================================
BACKTEST DTT - Detector de Tunelamento Topológico
Backtest com Dados Historicos REAIS
================================================================================

Este script executa backtest do indicador DTT (Detector de Tunelamento Topológico)
usando dados historicos REAIS do mercado Forex.

VERSÃO CORRIGIDA - SEM LOOK-AHEAD BIAS
======================================
- Direção baseada em barras FECHADAS (não usa momentum_direction do indicador)
- Entrada no OPEN da próxima barra
- Stop/Take consideram gaps
- Cálculos topológicos usam apenas dados passados

IMPORTANTE: Nenhuma simulacao - apenas dados reais do mercado.
Isso envolve dinheiro real, entao a precisao e crucial.

O indicador DTT detecta "Eventos de Tunelamento" usando:
1. Embedding de Takens (reconstrução do espaço de fase)
2. Homologia Persistente (detectar ciclos topológicos)
3. Equação de Schrödinger Financeira (probabilidade de tunelamento)

Uso:
    python -m backtesting.dtt.backtest [--days DAYS] [--symbol SYMBOL]
"""
...
from strategies.alta_volatilidade import DTTStrategy


def create_dtt_strategy(
    min_prices: int = 150,
    ...
    """
    Cria instancia da estrategia DTT com parametros customizados

    Args:
        min_prices: Minimo de precos para analise
        stop_loss_pips: Stop loss em pips
        take_profit_pips: Take profit em pips
        persistence_entropy_threshold: Threshold para entropia de persistência
        tunneling_probability_threshold: Threshold para probabilidade de tunelamento
        min_signal_strength: Força mínima do sinal
        direction_lookback: Barras para determinar direção (SEM look-ahead)

    Returns:
        Instancia de DTTStrategy
    """
    return DTTStrategy(
...
def run_dtt_backtest(
    ...
    """
    Executa backtest do DTT com dados REAIS
...
def run_optimization(
    ...
    """
    Executa otimizacao de parametros do DTT
    ...
    use backtesting.dtt.optimizer ao inves.
    ...
    """
    print("\n" + "=" * 70)
    print("  OTIMIZACAO DTT - BUSCA DE MELHORES PARAMETROS")
    print("  AVISO: Use optimizer.py para otimizacao com validacao robusta")
    print("=" * 70)

    # Parametros a testar
    stop_loss_values = [20.0, 25.0, 30.0, 35.0]
    take_profit_values = [40.0, 50.0, 60.0, 75.0]
    min_prices_values = [100, 150, 200]
...
def generate_report(result: BacktestResult, filename: str = None):
    ...
    report = []
    report.append("=" * 70)
    report.append("  RELATORIO DE BACKTEST - DTT")
    report.append("  Detector de Tunelamento Topológico")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Backtest DTT - Detector de Tunelamento Topológico com dados REAIS (Versao Corrigida)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Exemplos:
    python -m backtesting.dtt.backtest                    # Backtest padrao
    python -m backtesting.dtt.backtest --days 60          # 60 dias de historico
    python -m backtesting.dtt.backtest --optimize         # Otimizacao de parametros
    python -m backtesting.dtt.backtest --symbol GBPUSD    # Outro par
...
Para otimizacao robusta com train/test split, use:
    ...
    parser.add_argument(
        ...
        type=float,
        default=25.0,
        help="Stop loss em pips (default: 25)"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=50.0,
        help="Take profit em pips (default: 50)"
    )
