...
"""
================================================================================
VALIDACAO DE OVERFITTING - PRM (usando Yahoo Finance)
...
def download_forex_data(symbol: str, start_date: str, end_date: str, interval: str = "1h") -> list:
    """Baixa dados do Yahoo Finance"""
    print(f"\n  Baixando dados de {symbol} via Yahoo Finance...")
    print(f"  Periodo: {start_date} a {end_date}")

    # Yahoo Finance usa formato diferente para forex
    ...
    for idx, row in df.iterrows():
        bars.append(Bar(
            ...
            low=row['Low'],
            close=row['Close'],
            volume=row['Volume'] if 'Volume' in row else 1.0
        ))

    print(f"  Barras carregadas: {len(bars)}")
    if bars:
        print(f"  Periodo real: {bars[0].timestamp.date()} a {bars[-1].timestamp.date()}")
...
class OverfitValidator:
    ...
    def load_data(self, start_date: str, end_date: str):
        ...
        self.bars = download_forex_data(self.symbol, start_date, end_date, "1h")
...
def main():
    ...
    print(f"  Periodo de validacao: {start} a {end} (~12 meses)")
    ...
    print_results(result1, f"{start} a {end}")
    ...
    print_results(result2, f"{start} a {end}")
    ...
    if result1['trades'] == 0:
        ...
        print(f"    Os parametros sao muito restritivos fora do periodo de otimizacao.")
    elif result1['pf'] < 1.0:
        print(f"    OVERFITTED! PF caiu para {result1['pf']:.2f} (era 813,256)")
        print(f"    Esta configuracao NAO e viavel para trading real.")
    elif result1['pf'] < 2.0:
        print(f"    POSSIVEL OVERFIT. PF degradou significativamente para {result1['pf']:.2f}")
    else:
    ...
    if result2['trades'] == 0:
        ...
        print(f"    Degradacao de {abs(pf2_degradation):.0f}% no PF. Precisa mais analise.")
