#!/usr/bin/env python3
"""
RHHF (Ressonador Hilbert-Huang Fractal) - Teste de Otimização

Baseado em:
- EEMD (Ensemble Empirical Mode Decomposition)
- Transformada de Hilbert (frequência/amplitude instantânea)
- Detector de Chirp (ressonância)
- Análise Fractal da frequência
"""
...
def run_rhhf_backtest(bars: List[Bar], sl_pips: float, tp_pips: float, cooldown: int,
                      n_ensembles: int = 20, fractal_th: float = 1.2,
                      min_conditions: int = 2, long_only: bool = True,
                      start_pct: float = 0, end_pct: float = 100) -> Dict:
    """Backtest do RHHF com configuração leve"""
    n = len(bars)
    ...
    rhhf = RessonadorHilbertHuangFractal(
        ...
        chirp_threshold=0.0,
        fractal_threshold=fractal_th
    )

    signals = []
    min_bars = 120

    # Iterar a cada 10 barras para eficiência (EEMD é lento)
    for i in range(min_bars, len(closes) - 1, 10):
        try:
            prices_up_to_i = closes[:i]
            result = rhhf.analyze(prices_up_to_i)

            signal = result['signal']
            confidence = result['confidence']
            chirp_detected = result['chirp_detected']
            fractal_trigger = result['fractal_trigger']
            conditions = result['signal_details']['conditions_met']
...
def main():
    print("=" * 80)
    print("  RHHF (RESSONADOR HILBERT-HUANG FRACTAL) - OTIMIZACAO")
    print("  Baseado em: EEMD, Hilbert Transform, Chirp, Fractal Analysis")
    print("=" * 80)

    # Testar H4 (menos barras, mais eficiente)
    ...
    print("\n" + "=" * 80)
    print("  OTIMIZACAO DE PARAMETROS")
    print("=" * 80)

    configs = [
        {'ensembles': 15, 'fractal': 1.1, 'conds': 2},
        {'ensembles': 15, 'fractal': 1.2, 'conds': 2},
        {'ensembles': 20, 'fractal': 1.2, 'conds': 2},
        {'ensembles': 20, 'fractal': 1.3, 'conds': 2},
        {'ensembles': 25, 'fractal': 1.2, 'conds': 2},
    ]

    print(f"\n  {'Ens':>4} | {'Frac':>5} | {'Trades':>6} | {'WR%':>6} | {'Edge%':>7} | {'PnL':>8} | {'PF':>5} | Status")
    print("  " + "-" * 65)
    ...
    for c in configs:
        print(f"  Testando ensembles={c['ensembles']}, fractal={c['fractal']}...", end=" ", flush=True)
        result = run_rhhf_backtest(
            bars, sl, tp, cooldown,
            n_ensembles=c['ensembles'],
            fractal_th=c['fractal'],
            min_conditions=c['conds'],
            long_only=True
        )

        if result['total'] >= 5:
            status = "[OK]" if result['edge'] > 0 and result['pf'] > 1.0 else "[FAIL]"
            print(f"\r  {c['ensembles']:>4} | {c['fractal']:>5} | {result['total']:>6} | {result['wr']:>5.1f}% | "
            ...
            print(f"\r  {c['ensembles']:>4} | {c['fractal']:>5} | {result['total']:>6} | {'N/A':>6} | {'N/A':>7} | {'N/A':>8} | {'N/A':>5} | [SKIP]")
    ...
    print(f"\n  Melhor: ensembles={best['config']['ensembles']}, fractal={best['config']['fractal']}")
    ...
    for i in range(4):
        ...
        print(f"  Testando fold Q{i+1}...", end=" ", flush=True)
        fold = run_rhhf_backtest(
            bars, sl, tp, cooldown,
            n_ensembles=best['config']['ensembles'],
            fractal_th=best['config']['fractal'],
            min_conditions=2,
            long_only=True,
        ...
        fold['period'] = f"Q{i+1}"
        folds.append(fold)
        print(f"done ({fold['total']} trades)")
    ...
    # Salvar config
    config = {
        "strategy": "RHHF-HilbertHuangFractal-H4",
        "symbol": SYMBOL,
        "periodicity": "H4",
        ...
        "parameters": {
            "n_ensembles": best['config']['ensembles'],
            "fractal_threshold": best['config']['fractal'],
            "min_conditions": 2,
            "noise_amplitude": 0.2,
            "mirror_extension": 30,
            "stop_loss_pips": sl,
    ...
    path = "/home/azureuser/EliBotCD/configs/rhhf_h4_optimized.json"
...
def save_failed_report():
    config = {
        "strategy": "RHHF-HilbertHuangFractal",
        "symbol": SYMBOL,
        "optimized_at": datetime.now(timezone.utc).isoformat(),
        "verdict": "REPROVADO",
        "reason": "Nenhuma configuração gerou trades suficientes"
    }
    path = "/home/azureuser/EliBotCD/configs/rhhf_final_verdict.json"
    with open(path, 'w') as f:
        json.dump(config, f, indent=2)
