#!/usr/bin/env python3
"""
Universal Indicator Optimizer V2
================================
Otimizador robusto com nomes de classes corretos
"""
...
def walk_forward(bars: List[Bar], signals: List[Tuple[int, int]],
    ...
    if len(results) < 2 or len(all_trades) < 8:
        return {'valid': False}
...
# ============================================================================
# INDICATOR SIGNAL GENERATORS - CORRECTED CLASS NAMES
# ============================================================================
...
def compute_dsg_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    dsg = DetectorSingularidadeGravitacional(
        ricci_collapse_threshold=params.get('ricci_threshold', -50500),
        tidal_force_threshold=params.get('tidal_threshold', 0.1),
...
def compute_dtt_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    dtt = DetectorTunelamentoTopologico(
        persistence_entropy_threshold=params.get('entropy_threshold', 0.5),
        tunneling_probability_threshold=params.get('tunneling_threshold', 0.15),
...
def compute_fifn_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    fifn = FluxoInformacaoFisherNavier(
        ...
        kl_lookback=params.get('kl_lookback', 10),
        reynolds_sweet_low=params.get('reynolds_low', 2300),
        reynolds_sweet_high=params.get('reynolds_high', 4000),
...
def compute_odmn_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    odmn = OracloDerivativosMalliavinNash(
        lookback_window=params.get('lookback_window', 100),
        fragility_threshold=params.get('fragility_threshold', 2.0),
        mfg_direction_threshold=params.get('mfg_threshold', 0.1)
    )
...
def compute_phm_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    phm = ProjetorHolograficoMaldacena(
        ...
        bond_dim=params.get('bond_dim', 8),
    ...
    for i in range(150, len(closes) - 1, step):
        try:
...
def compute_prm_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    prm = ProtocoloRiemannMandelbrot(
        ...
        hmm_threshold=params.get('hmm_threshold', 0.85),
...
def compute_stgk_signals(bars: List[Bar], params: dict) -> List[Tuple[int, int]]:
    ...
    stgk = SintetizadorToposGrothendieckKolmogorov(
        complexity_window=params.get('complexity_window', 100)
    )
...
INDICATORS = {
    'DSG': {
        ...
        'param_grid': {
            'ricci_threshold': [-50600, -50500, -50400],
            'tidal_threshold': [0.08, 0.1, 0.12],
            'lookback_window': [40, 50, 60],
            'step': [8, 10]
        }
    },
    'DTT': {
        ...
        'param_grid': {
            'entropy_threshold': [0.4, 0.5, 0.6],
            'tunneling_threshold': [0.12, 0.15, 0.18],
            'max_points': [150, 200],
            'step': [8, 10]
        }
    },
    'FIFN': {
        'name': 'Fluxo Informação Fisher-Navier',
        'compute_fn': compute_fifn_signals,
        'param_grid': {
            'window_size': [40, 50, 60],
            'kl_lookback': [8, 10, 12],
            'skewness_threshold': [0.4, 0.5, 0.6],
            'step': [8, 10]
        }
    },
    'ODMN': {
        'name': 'Oráculo Malliavin-Nash',
        'compute_fn': compute_odmn_signals,
        'param_grid': {
            'lookback_window': [80, 100, 120],
            'fragility_threshold': [1.8, 2.0, 2.2],
            'mfg_threshold': [0.08, 0.1, 0.12],
            'step': [8, 10]
        }
    },
    'PHM': {
        'name': 'Projetor Holográfico Maldacena',
        'compute_fn': compute_phm_signals,
        'param_grid': {
            'window_size': [100, 128, 150],
            'bond_dim': [6, 8, 10],
            'n_layers': [3, 4],
            'step': [8, 10]
        }
    ...
    'PRM': {
        'name': 'Protocolo Riemann-Mandelbrot',
        'compute_fn': compute_prm_signals,
        'param_grid': {
            'n_states': [3],
            'hmm_threshold': [0.80, 0.85, 0.90],
            'curvature_threshold': [0.08, 0.1, 0.12],
            'lookback_window': [80, 100, 120],
            'step': [8, 10]
        }
    },
        ...
        if verbose:
            ...
            for combo, signals in signals_cache.items():
                if len(signals) >= 5:
                    ...
                    if r['total'] >= 5:
                        print(f"    {r['total']:3} trades, WR={r['wr']:.1f}%, Edge={r['edge']:+.1f}%, PF={r['pf']:.2f}")
...
def main():
    parser = argparse.ArgumentParser(description='Universal Indicator Optimizer V2')
    ...
    print("=" * 70)
    print("  UNIVERSAL INDICATOR OPTIMIZER V2")
    print("=" * 70)
