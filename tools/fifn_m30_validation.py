#!/usr/bin/env python3
"""
FIFN M30 Walk-Forward Validation
===============================================================================
Validacao rigorosa do melhor timeframe (M30) com walk-forward 3 folds.
"""
...
def run_fold(bars: List[Bar], start_pct: float, end_pct: float) -> dict:
    ...
    try:
        ...
        return {'total': 0, 'wr': 0, 'edge': 0, 'pnl': 0, 'pf': 0, 'error': str(e)}
...
def main():
    print("=" * 70)
    print("  FIFN M30 WALK-FORWARD VALIDATION")
    print("=" * 70)

    # Baixar mais barras para M30 (~6 meses)
    bars = download_bars("M30", 6000)
    if len(bars) < 1000:
    ...
    criteria = [
        ...
        (">=2 folds positivos (de 3)", positive_folds >= 2),
        ("Total trades >= 15", total_trades >= 15),
        ("Profit Factor agregado > 1.0", agg_pf > 1.0 if total_trades > 0 else False),
    ]
