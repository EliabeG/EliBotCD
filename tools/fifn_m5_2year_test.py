#!/usr/bin/env python3
"""
FIFN M5 - 2 Year Comprehensive Test
===============================================================================
Teste completo do FIFN M5 com 2 anos de dados historicos.
"""
...
TP_PIPS = 16.0
WINDOW_SIZE = 30
COOLDOWN = 6  # 30 minutos entre trades
SPREAD_PIPS = 1.2
SLIPPAGE_PIPS = 0.5
PIP_VALUE = 0.0001
...
# Modo LONG-only (baseado em analise anterior)
LONG_ONLY = True

API_BASE = "https://ttlivewebapi.fxopen.net:8443/api/v2/public/quotehistory"
SYMBOL = "EURUSD"

# M5: 288 barras/dia * 260 dias = ~75000 barras por ano
# Vamos baixar o maximo possivel
BARS_TARGET = 75000
    ...
    while total < count:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=30, context=ctx) as response:
                ...
                batch = data.get("Bars", [])
                if not batch:
                    print(f"    Sem mais dados em {total} barras")
                    break
                for b in batch:
...
def run_backtest(bars: List[Bar], start_pct: float = 0, end_pct: float = 100,
    ...
    for idx, direction, timestamp in signals:
        ...
        trade_result = None
        exit_price = 0
        for j in range(idx + 2, min(idx + 500, n)):  # M5: mais barras para atingir TP/SL
            bar = bars[j]
            if direction == 'LONG':
                if bar.low <= sl:
...
def main():
    print("=" * 80)
    print("  FIFN M5 - 2 YEAR COMPREHENSIVE TEST (LONG-ONLY)")
    print("=" * 80)

    # Baixar dados M5
    bars = download_bars("M5", BARS_TARGET)
