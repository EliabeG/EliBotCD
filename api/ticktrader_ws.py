...
class TickTraderFeed:
    ...
    def _generate_signature(self, timestamp: str) -> str:
        """Gera assinatura HMAC para autenticação (Base64 conforme API TickTrader)"""
    ...
    async def _process_messages(self):
        ...
        while self.connected and self.ws and self.ws.state.name == 'OPEN': # Verificando estado corretamente
            try:
                ...
                if event_type == 'Tick' or event_type == 'FeedTick':
                    await self._handle_tick(data)
                elif event_type == 'Depth' or event_type == 'FeedDepth':
                    await self._handle_depth(data)
                elif event_type == 'FeedSubscribe':
                    # Processar snapshot inicial da subscrição
                    await self._handle_feed_subscribe(data)
                    ...
                    # A API retorna 'Info': 'ok' para sucesso, não 'Success': True
                    if result.get("Info") == "ok" or result.get("Success", False):
    ...
    async def _handle_feed_subscribe(self, data: Dict[str, Any]):
        """Processa resposta de subscrição que contém snapshot inicial"""
        try:
            result = data.get('Result', {})
            snapshots = result.get('Snapshot', [])

            for snapshot in snapshots:
                # Cada snapshot é um tick inicial
                try:
                    tick = TickData(snapshot)
                    self.tick_buffer.append(tick)
                    logger.info(f"Snapshot inicial recebido: {tick.symbol} Bid={tick.bid:.5f} Ask={tick.ask:.5f}")

                    # Também atualizar DOM se houver profundidade
                    if snapshot.get('Bids') or snapshot.get('Asks'):
                        dom = DOMSnapshot(snapshot)
                        if dom.symbol:
                            self.dom_buffer[dom.symbol] = dom

                except Exception as e:
                    logger.error(f"Erro ao processar snapshot: {e}")

        except Exception as e:
            logger.error(f"Erro ao processar FeedSubscribe: {e}")
    ...
    async def _send_message(self, message: Dict[str, Any]): # Usar Any
        ...
        if self.ws and self.connected and self.ws.state.name == 'OPEN': # Verificando estado corretamente
            try:
        ...
        async with self._reconnect_lock:
            ...
            for attempt in range(max_retries):
                ...
                try:
                    ...
                    if self.ws and self.ws.state.name != 'CLOSED': # Fechar conexão antiga se existir
                        await self.ws.close()
    ...
    def is_connected(self) -> bool:
        ...
        return self.connected and self.ws is not None and self.ws.state.name == 'OPEN'
...
class TickTraderTrade:
    ...
    def _generate_signature(self, timestamp: str) -> str:
        """Gera assinatura HMAC (Base64 conforme API TickTrader)"""
        message = f"{timestamp}{CONFIG.WEB_API_TOKEN_ID}{CONFIG.WEB_API_TOKEN_KEY}"
        signature = base64.b64encode(
            hmac.new(
                CONFIG.WEB_API_TOKEN_SECRET.encode('utf-8'),
    ...
    async def _load_account_info(self):
        ...
        if response and response.get('Result') and not response.get('Error'):
            ...
            logger.info(f"Informações da conta carregadas: Id {self.account_info.get('Id')}, "
                       f"Balanço: {self.account_info.get('Balance')} {self.account_info.get('BalanceCurrency')}")
        else:
    ...
    async def create_market_order(self, symbol: str, side: str, volume: float, strategy_comment: Optional[str]=None) -> Optional[Dict[str, Any]]:
        ...
        # Converter volume de lotes para unidades (1 lote = 100000 unidades para forex)
        ...
        if response and response.get('Result') and not response.get('Error'):
            ...
            if 'Id' in trade:
                self.orders[trade['Id']] = trade
                logger.info(f"Ordem a mercado criada: {trade['Id']} | {side} {volume} lotes ({amount_units} units) {symbol}")
                return trade
            else:
    ...
    async def create_limit_order(self, symbol: str, side: str, volume: float,
        ...
        if response and response.get('Result') and not response.get('Error'):
            ...
            if 'Id' in trade:
                self.orders[trade['Id']] = trade
                logger.info(f"Ordem limite criada: {trade['Id']} | {side} {volume} lotes {symbol} @ {price}")
                return trade
            else:
    ...
    async def modify_order(self, order_id: str,
        ...
        if new_volume is not None:
            params["Amount"] = new_volume

        # Não modificar se não houver nada para mudar além do Id
        if len(params) <= 1:
    ...
    async def close_position(self, position_id: str, volume: Optional[float] = None) -> bool: # Adicionado volume para fechamento parcial
        ...
        if response and response.get('Result') and not response.get('Error'):
            if volume is None or (position_id in self.positions and volume >= self.positions[position_id].get('Amount', float('inf'))):
    ...
    async def get_positions(self) -> List[Dict[str, Any]]:
        """Obtém posições/trades abertas (usa Trades para contas Gross, Positions para Net)"""
        if not self.is_connected():
            logger.error("Não conectado. Não é possível obter posições.")
            return []

        # Verificar tipo de conta - Gross usa Trades, Net usa Positions
        account_type = self.account_info.get('AccountingType', 'Gross')

        if account_type == 'Gross':
            # Para contas Gross, usar Trades
            trades_msg = {
                "Id": self._get_next_id(),
                "Request": "Trades"
            }
            response = await self._send_and_wait(trades_msg)

            if response and response.get('Result') and not response.get('Error'):
                result = response['Result']
                trades_list = result if isinstance(result, list) else result.get('Trades', [])
                # Filtrar apenas trades ativos (não ordens pendentes)
                active_trades = [t for t in trades_list if t.get('Status') in ['Filled', 'PartiallyFilled', 'Calculated'] or t.get('Type') == 'Position']
                self.positions = {t['Id']: t for t in active_trades if 'Id' in t}
                return active_trades
            else:
                error_details = response.get('Error', response.get('Result', {})) if response else "Sem resposta"
                logger.error(f"Falha ao obter trades: {error_details}")
                return []
        else:
            # Para contas Net, usar Positions
            positions_msg = {
                "Id": self._get_next_id(),
                "Request": "Positions"
            }
            ...
            if response and response.get('Result') and not response.get('Error'):
                ...
                if isinstance(result, list):
                    self.positions = {p['Id']: p for p in result if 'Id' in p}
                    return result
                elif isinstance(result, dict):
                    positions_list = result.get('Positions', [])
                    self.positions = {p['Id']: p for p in positions_list if 'Id' in p}
    ...
    async def _send_and_wait(self, message: Dict[str, Any], timeout: float = 10.0) -> Optional[Dict[str, Any]]: # Usar Any
        ...
        # Verificar apenas conexão WS, não autenticação (para permitir envio do login)
        if not (self.connected and self.ws is not None and self.ws.state.name == 'OPEN'):
            logger.error("Não conectado ao WebSocket, _send_and_wait falhou.")
            return None
    ...
    def is_connected(self) -> bool:
        ...
        return self.connected and self.authenticated and self.ws is not None and self.ws.state.name == 'OPEN'
