...
class Position:
    ...
    pnl: float = 0.0  # PnL realizado/flutuante
    trailing_stop: bool = False  # Se trailing stop está ativo
    metadata: Dict[str, Any] = field(default_factory=dict) 
...
class BaseStrategy(ABC):
    ...
    # certificando-se de que a ordem dos campos em Signal e Position est� correta
    ...
    def _get_symbol_from_context(self, market_context: Dict[str, Any]) -> str:
        """Obtém símbolo do contexto de mercado de forma segura."""
        # Primeiro tenta obter direto do contexto
        symbol = market_context.get('symbol')
        if symbol:
            return symbol
        # Se não, tenta obter do tick (que pode ser objeto ou dict)
        tick = market_context.get('tick')
        if tick:
            if hasattr(tick, 'symbol'):
                return tick.symbol or CONFIG.SYMBOL
            elif isinstance(tick, dict):
                return tick.get('symbol', CONFIG.SYMBOL)
        return CONFIG.SYMBOL

    def _get_pip_size(self, market_context: Dict[str, Any]) -> float:
        """Obtém tamanho do pip baseado no símbolo."""
    ...
    def _get_prices_from_context(self, market_context: Dict[str, Any], price_type: str = 'mid', lookback: Optional[int]=None) -> np.ndarray:
        ...
        # Mapeamento de fallback para tipos de preço não disponíveis em dados tick
        # high -> ask (melhor aproximação), low -> bid (melhor aproximação)
        fallback_map = {'high': 'ask', 'low': 'bid', 'open': 'mid', 'close': 'mid'}

        for tick_item in ticks_list_any:
            price_val = 0.0
            if hasattr(tick_item, price_type):
                price_val = getattr(tick_item, price_type, 0.0)
                # Se o atributo existe mas é 0 ou None, tentar fallback
                if not price_val and price_type in fallback_map:
                    price_val = getattr(tick_item, fallback_map[price_type], 0.0)
            elif isinstance(tick_item, dict):
                price_val = tick_item.get(price_type)
                # Se não existe ou é 0/None, tentar fallback
                if not price_val:
                    if price_type in fallback_map:
                        price_val = tick_item.get(fallback_map[price_type], tick_item.get('mid', 0.0))
                    else:
                        price_val = tick_item.get('mid', 0.0)
