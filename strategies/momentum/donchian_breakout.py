...
class DonchianBreakoutStrategy(BaseStrategy):
    ...
    async def evaluate_exit_conditions(self, open_position: Position,
        ...
        """Avalia condições de saída para uma posição aberta (método abstrato requerido)"""
        # Obter preço atual do contexto de mercado
        current_price = None
        if 'last_tick' in market_context:
            tick = market_context['last_tick']
            current_price = (tick.bid + tick.ask) / 2 if hasattr(tick, 'bid') else tick.get('mid')
        elif 'current_price' in market_context:
            current_price = market_context['current_price']

        if current_price is None:
            return None

        # Usar o método calculate_exit_conditions existente
        return await self.calculate_exit_conditions(open_position, current_price)