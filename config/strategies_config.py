...
class StrategyConfig:
    ...
    # Configurações para estratégias otimizadas via backtest
    OPTIMIZED_CONFIG: Dict[str, Any] = {
        'priority': 1,  # Alta prioridade para seleção de estratégias
        'enabled': True,  # Habilitadas por padrão
        'use_dynamic_stops': False,  # Usar SL/TP fixos (otimizados)
        'allow_partial_exits': False,  # Saída total apenas

        # OptimizedRSI settings
        'rsi_strategy': {
            'enabled': True,
            'rsi_period': 14,
            'rsi_oversold': 30,
