...
class TradingOrchestrator:
    ...
    async def _initialize_regime_detector(self): 
        ...
        try:
            ...
            if not self.regime_detector.is_trained:
                ...
                start_date = now - timedelta(days=days_for_train)
    ...
    async def run(self):
        ...
        # Seleção inicial de estratégias (antes de receber dados)
        logger.info("Realizando selecao inicial de estrategias...")
    ...
    async def _process_market_data(self):
        ...
        while self.running:
            try:
                ...
                self.tick_count += 1
                # Converter TickData para dict antes de armazenar
                tick_dict = tick.to_dict() if hasattr(tick, 'to_dict') else {
                    'symbol': tick.symbol,
                ...
                await self.data_manager.store_tick(tick_dict)
    ...
    async def _select_active_strategies(self):
        ...
        # Usar regime padrão se não detectado
        effective_regime = self.current_regime
        if not effective_regime:
            effective_regime = MarketRegime.RANGE  # Regime padrão conservador
            logger.info(f"Regime nao detectado, usando regime padrao: {effective_regime}")

        logger.info(f"Selecionando estrategias para regime: {effective_regime}")
        suitable_strategies = []
        min_score = getattr(CONFIG, 'MIN_STRATEGY_SCORE_TO_ACTIVATE', 0.3)

        for name, strategy_instance in self.strategies.items():
            if effective_regime in strategy_instance.suitable_regimes:
                score = self.strategy_scores.get(name, 0.5)  # Score inicial de 0.5 para novas estratégias
                # Permitir estratégias com score >= 0 inicialmente (sem trades ainda)
                if score >= 0 or name not in self.strategy_scores:
                    suitable_strategies.append((name, max(score, 0.5)))
    ...
    async def _build_market_context(self, current_tick: TickData) -> Dict[str, Any]: 
        ...
        return {
            "tick": current_tick,
            "symbol": current_tick.symbol if current_tick else CONFIG.SYMBOL,
            "bid": current_tick.bid if current_tick else 0.0,
            "ask": current_tick.ask if current_tick else 0.0,
            "mid": current_tick.mid if current_tick else 0.0,
